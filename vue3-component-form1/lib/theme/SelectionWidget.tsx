...
const SelectionWidget: SelectionWidget = defineComponent({
  ...
  setup(props) {
    ...
    return () => {
      const { options } = props;
      return (
        <select multiple={true} v-model={changeValueRef.value}>
          {options.map((opValue) => {
            return <option value={opValue.value}>{opValue.key}</option>;
          })}
        </select>
      );
    };
